...
export class PayrollsService {
  ...
  async process(user: { id: number; role: Role }, dto: ProcessPayrollDto) {
    const start = utcDate(new Date(dto.payPeriodStart));
    const end = utcDate(new Date(dto.payPeriodEnd));
    if (
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      for (const employee of employees) {
        const baseSalary = employee.job_title?.salary_range_min;
        if (!baseSalary) continue;
        const invalidDays = employee.leaveRequests
          .filter((leave) => leave.status !== LeaveStatus.APPROVED_BY_HR)
          .reduce(
            (sum, leave) =>
              sum + overlapDays(leave.startDate, leave.endDate, start, end),
            0,
          );
        ...
        processed.push(
          await tx.payroll.upsert({
            ...
            create: {
              ...
              baseSalary,
              deductions,
              totalSalary: baseSalary.minus(deductions),
              payPeriodStart: start,
              payPeriodEnd: end,
            },
            update: {
  ...
  async list(user: { id: number; role: Role }) {
    ...
    return this.prisma.payroll.findMany({
      where,
      orderBy: [{ payPeriodStart: 'desc' }, { id: 'desc' }],
    });
  }
}
